...
class Angry_Dice:
    ...
    def valid_check(self):
        if self.currentStage == 1:
            if (self.angryDieA.__repr__() != '1'
                and self.angryDieA.__repr__() != '2'):
                self.invalidFlagA = True
            if (self.angryDieB.__repr__() != '1'
                and self.angryDieB.__repr__() != '2'):
                self.invalidFlagB = True
        if self.currentStage == 2:
            if self.angryDieA.__repr__() != 'ANGRY' \
                and self.angryDieA.__repr__() != '4':
                self.invalidFlagA = True
            if self.angryDieB.__repr__() != 'ANGRY' \
                    and self.angryDieB.__repr__() != '4':
                self.invalidFlagB = True
        if self.currentStage == 3:
            if self.angryDieA.__repr__() != '5':
                self.invalidFlagA = True
            if self.angryDieB.__repr__() != '5':
                self.invalidFlagB = True
    #as long as they aren't holding an invalid die, play the game
    def can_play(self):
    ...
    def main(self):
        ...
        print("Welcome to Angry Dice! Roll the two dice until you get thru the "
              ...
              " die needed for your current stage and just roll the other one, "
              "but beware!\nIf you ever get 2 ANGRY's at once, you have to "
              "restart to Stage 1!\nAlso, you can never lock a 6! That's "
              "cheating!\n To roll the dice, simply input the name of the die "
              "you want to roll.\nTheir names are a and b.\nTo roll the dice, "
              "simply input the name of the die you want to roll.\n Their names"
              " are a and b.\n")
        ...
        #Enter body of game
        while True:
            #see if they can play, or if they're breaking the rules
            self.can_play()
